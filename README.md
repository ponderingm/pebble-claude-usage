...
縦一列にテキストを積む一覧表スタイルをやめて、上下の細い使用率バーの内側を **Header / Body /
Footer / Status** の4領域に分けている。配色はダークスレート（黒背景・白文字、使用率バーと天気
アイコンの色だけアクセントとして残す）。

- **使用率バー**（画面最外殻、上下）: 上＝5時間枠、下＝7日枠。文字は乗せず、使用率に応じて
  緑（70%未満）→オレンジ（70-90%）→赤（90%以上）に変化する色の帯だけを表示
...
- **Footer**（5H | 7D、横並び）: 使用率バーから追い出した数値情報 —
  左に5時間枠の%とリセットまでのカウントダウン、右に7日枠の同情報
