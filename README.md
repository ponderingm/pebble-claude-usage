...
横線2本で「天気/バス」「日付/時刻」「電池/Bluetooth」の3段に分けたバンド構成にしている（縦一列に
テキストを積むと単なる一覧表に見えてしまうため）。配色はダークスレート（黒背景・白文字、使用率
バーと天気アイコンの色だけアクセントとして残す）。

- **時刻**: 24時間表示、中段に大きく表示（`clock_is_24h_style()` の設定に関わらず常に24h表示）
- **日付**: `MM-DD (Day)` 形式、時刻の上に表示
- **電池残量**: `BatteryStateService` から取得（通信不要）。電池の形をしたアイコンで残量を表示し、
  20%以下で赤くなる
- **Bluetooth**: `ConnectionService` から取得し、**切断時のみ**丸に斜線のアイコンを表示（通信不要）。
  接続中は電池アイコンを段の中央に寄せる
- **Claude Usage**: 画面最外殻の四角枠バーグラフ。上半分＝5時間枠、下半分＝7日枠。使用率に応じて
  緑（70%未満）→オレンジ（70-90%）→赤（90%以上）に変化し、枠内に%とリセットまでのカウントダウンを1行で表示
