...
- **時刻**: 24時間表示、画面中央に大きく表示（`clock_is_24h_style()` の設定に関わらず常に24h表示）
- **日付**: `MM-DD (Day)` 形式
- **電池残量**: `BatteryStateService` から取得（通信不要）
- **Bluetooth**: `ConnectionService` から取得し、**切断時のみ** `BT!` を表示（通信不要）
- **Claude Usage**: 画面最外殻の四角枠バーグラフ。上半分＝5時間枠、下半分＝7日枠。使用率に応じて
  緑（70%未満）→オレンジ（70-90%）→赤（90%以上）に変化し、枠内にリセットまでのカウントダウンも表示
- **天気・気温**: [Open-Meteo](https://open-meteo.com/)（APIキー不要）から現在地の気温と天気を取得し、
  簡易図形アイコン（晴れ・曇り・雨・雪・霧・雷）で表示
- **通勤バス**: 都営バス**業10系統**（本所四丁目⇔木場四丁目）の次発までの残り分数を表示。
  スマホGPSの現在地と自宅・職場座標の近さから行き帰りを自動判定する
