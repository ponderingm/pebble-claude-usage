...
- **使用率バー**（画面最外殻、上下、薄め）: 上＝5時間枠、下＝7日枠。使用率に応じて
  ...
  カウントダウンを1行で重ねて表示。上段バーの左上には電池アイコンを重ねて表示する
  （アイコンのみなので帯に重なっても支障がないため）
...
- **Footer**: 今のところ意図的に空欄（`draw_footer_band` は予約枠として何もしない）。
  電池表示を使用率バー上に移したことで広くなった、今後ここに別の情報を足せるように場所だけ確保してある
- **Status**（Bluetooth、アイコンのみ）: `ConnectionService` から取得したBluetooth接続状態を
  **切断時のみ**丸に斜線のアイコンで表示（通信不要）
- **電池残量**: `BatteryStateService` から取得（通信不要）。電池の形のアイコンで表示し、
  20%以下で赤くなる（パーセント文字は出さない）
